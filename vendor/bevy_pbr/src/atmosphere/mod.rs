...
pub struct Atmosphere {
    ...
    /// (atmosphere units — the world position times `scene_units_to_m`). Upstream fixes the centre
    /// at `(0, -bottom_radius, 0)` under the world origin and takes world Y as up, which is a flat
    /// world; a spherical world with the eye anywhere states the centre here. [`Self::earthlike`]
    /// keeps upstream's value, so upstream scenes render unchanged.
    ///
    /// units: m
...
impl Atmosphere {
    pub fn earthlike(medium: Handle<ScatteringMedium>) -> Self {
        ...
        Self {
            ...
            medium,
            planet_center: Vec3::new(0.0, -EARTH_BOTTOM_RADIUS, 0.0),
        }
    }
}
...
impl ExtractComponent for Atmosphere {
    ...
    fn extract_component(item: QueryItem<'_, '_, Self::QueryData>) -> Option<Self::Out> {
        Some(ExtractedAtmosphere {
            ...
            planet_center: item.planet_center,
        })
    }
}
