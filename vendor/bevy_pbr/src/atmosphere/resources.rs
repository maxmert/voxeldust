...
pub struct GpuAtmosphere {
    ...
    pub top_radius: f32,
    /// VOXELDUST PATCH (planet centre): the planet's centre in the camera's world, in metres.
...
pub fn prepare_atmosphere_uniforms(
    ...
    for (entity, atmosphere) in atmospheres {
        commands.entity(entity).insert(GpuAtmosphere {
            ...
            planet_center: atmosphere.planet_center,
        });
    }
    Ok(())
}
...
pub(super) fn prepare_atmosphere_transforms(
    ...
    for (entity, view, atmosphere, settings) in &views {
        ...
        // VOXELDUST PATCH (planet centre): the local up is the direction from the stated planet
        // centre to the eye, never the world's Y. Upstream's `Vec3A::Y` is this expression with the
        // centre fixed at (0, -bottom_radius, 0) under the origin.
        let eye_m = world_from_view.translation * settings.scene_units_to_m;
        let atmo_y = (eye_m - Vec3A::from(atmosphere.planet_center))
            .try_normalize()
            .unwrap_or(Vec3A::Y);
        let reject = |v: Vec3A| v - atmo_y * v.dot(atmo_y);
        let atmo_z = reject(camera_z)
            .try_normalize()
            .unwrap_or_else(|| reject(camera_y).normalize());
