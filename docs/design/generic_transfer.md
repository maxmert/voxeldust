...
1. The handoff blob carries a per-victim **`applied_damage: BTreeSet<DamageEventId>`** (small, bounded, pruned by `hit_tick` age) AND the authoritative post-drain health. *(Type note: NOT `HashSet` — the sim/node clippy `disallowed-types` bans the default-hasher `std::collections::HashSet`, and this set CROSSES the transfer barrier into the byte-identical-replay surface, so it must be deterministic: use `BTreeSet`, or `vd_core::collections::DetHashSet` if O(1) membership is needed under firefight load.)* At FLUSH the source **drains and applies all pending `DamageEvent`s with `hit_tick < freeze_tick`** into the health it ships, and includes their `event_id`s in `applied_damage`.
