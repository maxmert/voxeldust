...
- **The owed items (from the audit, each with its fix):**
  ...
  2. **Unresolved-dest re-drive (audit H2) — SITE CORRECTED (goal-audit-e2e wf_6b221e10, C1):** the durable `CrossingRequest` is emitted ONCE (`Entry::Vacant`); a DELIVERED-but-unresolved dest (`head(Realm(to))` transiently absent at P4/P5) STRANDS the entity. **The `Entry::Occupied` re-drive the prior text prescribed is UNREACHABLE** — `should_commit` (`geometry.rs:642`) is a STRICT one-tick rising edge, so `fan_out_crossing` never re-runs for a statically-dwelling stranded dot. FIX (owed, 3f-D4): a NEW per-tick **latch-scan** system `redrive_stranded_crossings` over `RequestInFlight`, scheduled `.after(evaluate_realm_boundaries)`, re-emitting the SAME latched request when `local_tick - last_commit_tick >= request_ttl_ticks` (`ttl==0` default = INERT). Load-bearing design decision: widen `CrossingState`/the latch to carry `{to_realm, session, subject_fence}` at emit time (the scan can't recover them from `RequestInFlight`'s `EntityId→TransferId` alone). WIRE `request_ttl_ticks` (audit L8 — do NOT drop).
