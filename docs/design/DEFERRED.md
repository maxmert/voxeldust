...
### D-RLM-2 🟧 The durable RLM demand-ledger snapshot (`StoreKey::Rlm` tag 6) — DEFERRED with machine-checked evidence (RLM Step 4b; spec `scripts/rlm_step4_persistence_spec.md`)
The orchestrator's RAM `DemandLedger` (`crates/sim/src/rlm.rs`) is NOT persisted: on a kill-9 rebuild it comes up EMPTY and self-heals from the re-asserted (`ReDriven`, every tick) demands within ~1 demand round-trip. The RLM Step-4a crash-recovery FREEZE (`RlmTuning::recovery_grace_ticks`, armed in `saga_runtime::rehydrate` from the RAW recovered ceiling, applied via `RlmReconcilerRes::arm_quiesce` in `register_orchestrator_with_store`) blocks teardown for one full demand-TTL past the recovered ceiling, bridging the window until demands re-accrue. A durable snapshot (a `StoreKey::Rlm(RealmPath)` tag-6 record + `LedgerCell` serde + dirty-cell staging into the D-6 group-commit barrier; full stage/restore/`draining_since=None` design in the spec §3) would save only ≈1 tick of re-spin latency and CANNOT replace the freeze (it can lose ≤1 off-tick batch). It is DEFERRED, not owed.
