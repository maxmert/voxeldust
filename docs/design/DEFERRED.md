...
- **Pin:** the `rehydrate` "drop partial" comment (`crates/node/src/rlm_spawn.rs`) names this entry. Flips 🟩 when 5e's kill-9-mid-spawn gate proves no-orphan/no-double-spawn.
...
- **WHAT is deferred:** the child-peer-view REFRESH policy. Two candidate mechanisms (JOINT user decision, per no-simplest-fix — do NOT pick unilaterally): **(a)** the reconciler re-spawns the descendant subtree when an ancestor's incarnation changes (reuses the existing spawn path, ZERO new wire — the likely HR-clean choice); **(b)** a runtime lane-create/re-key control path (NEW wire — needs joint investigation).
- **WHERE it lands + the gate:** RLM Step 5f (armed reconciler) with a **HARD gate** (not an assertion): ancestor crash+respawn while a descendant stays live → the descendant re-reaches the new ancestor within bounded ticks. Related: **D4** (first-contact `Unreliable` Input dropped before the freshly-spawned dest is learned by the gateway) is also a 5f HARD gate (drive a real re-home onto a fresh dest, assert zero input-drop across the commit — gate/buffer at the gateway or eager dest→gateway dial at boot).
