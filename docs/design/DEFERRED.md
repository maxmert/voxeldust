...
- **The owed items (from the audit, each with its fix):**
  ...
  4b-DEFERRED 🟧 **DOCK/UNDOCK NESTING (Slice 4b, its own design):** the OUTWARD/undock crossing rides a direction-BLIND single latch (`RequestInFlight: BTreeMap<EntityId, TransferId>` keyed by entity only), so a dock→undock interleave silently suppresses the undock and strands the entity (adversary **CRITICAL-1**, `scripts/slice4_design_adversary.md`). The fence-keyed re-parent (`DemoteCmd.new_parent`, `apply_reparent`) + the direction-keyed latch (`RequestInFlight` → `(Direction, TransferId)`, `crossing_transfer_id` gaining direction, `LatchedCrossing`/re-drive/`on_saga_demote` direction-matching) + the per-(entity,boundary) cooldown (#4) + stable `winner_ix` (#5) are ALL Slice 4b — real new machinery, NOT INERT shape. Slice 4a deliberately drives INWARD only to stay clear of this hazard. Design owed before the first real docked-ship boundary is planted.
