...
- **PROGRESS (5e-4, uncommitted):** the orphan-ADOPT path this entry's reaper reuses now EXISTS — `LaunchBackend::adopt` + `ProcLaunchBackend`'s `Slot::Orphan` + the `/whoami` cookie-probe (`is_alive`) + the signal-only `teardown`, with `SpawnCore::rehydrate` re-adopting each `pid:Some` survivor. Proven in the process gate's ADOPT+cookie-probe leg. The narrow no-ledger-row orphan (this entry) is the ONE case adopt does NOT cover (rehydrate has no row to adopt from) — its reaping is 5e-5's kill-9-mid-spawn sweep.
