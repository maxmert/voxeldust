...
### D-45 🟧 Containment realm-membership — owed slices + P4/P5/P8/P10 (task #135; C-1/C-2 `8ef619c`, C-3 landed 2026-07-16; design `scripts/containment_realm_membership_design.md`)
- **LANDED:** the pure containment layer (C-1/C-2) + the sim swap (C-3: detector rewrite, `RealmRegions`/`ContainmentProgress`/`RegionMembership`, deleted portal fns, ~40-test migration) — green + 100% region/branch, −764 lines. INERT in prod (`RealmRegions` default-empty; seed boot is C-5/C-6).
...
- **C-5** `guard_regions_nest` boot validator (exactly one `parent:None` root, unique `.realm`, no cycles, count ≤ `MAX_REGIONS`=64) + relax `guard_boundaries_in_realm`→own realm + ancestor chain. A rootless/oversized forest = boot fail-loud (the detector's defensive rootless no-op is covered by `a_rootless_region_forest_is_a_safe_no_op`).
- **C-6** the SEED-DERIVED bins BOOT: replace the INTERIM `vd_bins::regions_for_source_plant` adapter with `realm_regions_for(seed)` filtered to own+ancestors+neighbours, single-sourced with the client `--realm-boxes`; the 3-deep Universe⊃Galaxy⊃System⊃Planet round-trip gate (`escape_soi_lands_in_system_not_galaxy`, `symmetric_recross`). **SEMANTIC-DIVERGENCE DISCLOSURE (verify `wf_53d240e8`):** the interim adapter models the dest realm as a CHILD of the source (`parent = hosted_realm`) — a fixture stopgap that keeps `dual_cluster_crossing_smoke` green (it proves a born-inside CHILD re-home). The canonical `realm_regions_for` models a dest star-system as a DISJOINT SIBLING under the galaxy (`SYSTEM_B parent=GALAXY`, depth 2). So the interim smoke proves child re-home, NOT the escape-SOI-to-sibling round-trip — C-6 supplies that. **SEQUENCING:** C-5's `guard_regions_nest` + relaxed in-realm guard must land TOGETHER WITH the C-6 adapter removal (or explicitly tolerate the interim child-nesting), so the validator is never run against the non-canonical interim forest; and the adapter's `root_realm = System(0)` collides if a shard boots `realm_seed == 0` (a degenerate cyclic forest — `region_depth`'s hop-cap prevents a hang, `guard_regions_nest` rejects it at C-5; unreachable in the playground where `realm_seed == 7`).
