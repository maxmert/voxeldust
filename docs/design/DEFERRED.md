...
### D-43 🟧 Geometric transfer-TRIGGER owed items — the crossing PRODUCER + consumer landed (50dc45e/42d7b73), the abort DURABILITY + re-drive + scale hardening owed before task #133 plants the first real `RealmBoundaries` band (holistic audit `wf_3916069b`, 2026-07-13)
...
- **The owed items (from the audit, each with its fix):**
  ...
  2. **Unresolved-dest re-drive (audit H2):** the durable `CrossingRequest` is emitted EXACTLY ONCE (`Entry::Vacant`); `Entry::Occupied` is a pure suppress. A DELIVERED-but-unresolved dest (`head(Realm(to))` temporarily absent — a realm shard mid-lease / partitioned at P4/P5) counts `crossing_unresolved` and STRANDS the entity (transport retry covers lost-in-flight + the crash case where the in-RAM latch resets, but NOT delivered-but-unresolved). Fix: **WIRE `request_ttl_ticks`** (currently the dead field, audit L8 — do NOT drop it) — on `Entry::Occupied` with `now - last_commit_tick >= request_ttl_ticks`, RE-EMIT (the orchestrator's `sagas.contains_key` guard absorbs a duplicate that DID start; an unresolved one re-tries the head reads). Fold into 3f-D.
  3. **CrossingProgress re-arm on abort (audit L1):** clearing the latch is necessary but not sufficient — the dwell (`inward_ticks`/`was_member`/`last_commit_tick`) is never re-armed, so a still-dwelling entity won't re-request until it physically re-crosses. Reset/drop the `CrossingProgress` entry on abort-clear; add an abort-while-dwelling scenario. Fold into 3f-D.
  4. **Cooldown per-(entity,boundary) (audit L5):** the anti-thrash cooldown is per-ENTITY while the dwell is per-BOUNDARY — a legit back-to-back station→bay Authority crossing within `k_dwell` is dropped (the landed regression uses `Interest`, which arms no cooldown, so this is untested). Make the cooldown per-(entity,boundary) to match the dwell; add an Authority A-then-B regression. Fold into task #133 (Station/Bay).
  5. **`winner_ix` stable identity (audit L9):** `CrossingState.winner_ix` is a RAW slice index into `RealmBoundaries`; the dwell reset compares indices. Stable only while the registry is immutable — a P4/P5 dynamic reorder aliases dwell to a different boundary. Key the reset on a STABLE per-boundary id (NOT `RealmId` alone — coincident dock mouths share a realm). Part of task #133.
