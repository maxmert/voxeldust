...
- **STILL OWED (the `reanchor()` half — P5):** the D-38 doc (`capability.rs:14`) mandates ONE fixture forcing a
...
- **Owed (GPU pixel-capture walk re-base, task #149):** the `--triple` co-hosting cluster now THRASHES under
  ...
  interim CI walk gate, and the live `--forest` window is the interim VISUAL proof. The GPU pixel-capture walk
  proof (the client-observed `location`-label + Galaxy-box-containment capstone) is OWED a re-base onto the
  `--forest` node-per-realm cluster (a headless `client --capture --realm-boxes` over the six-shard forest).
...
### D-RLM-11 🟥 VU-AoI S2c — sibling RENDER to the traveller's client (the REQUIRED render companion to S2b; the "no-pop" bar for a neighbour realm is met only by S2b + S2c together) — vetted `vu-aoi-s2b-design` 2026-07-28
- **WHAT is deferred:** DRAWING the warmed neighbour realm on the traveller's screen while they are still inside their own realm. S2b (`9c5bcb5`/`290d816`/`dc56114`) makes the parent WARM the neighbour (spin its node/interior up ahead) by folding the up-relayed occupant as a proxy; it does NOT stream the neighbour's box outline to the client. The parent structurally cannot: render deltas route by `AccountId → the DOT's gateway` (`render_routes`, dots-only), a proxy has no route, and `OccupantInterest` carries no gateway. So the neighbour's outline appears at the CROSSING instant (when the traveller becomes a real dot the neighbour's shard/ancestors route to), not on approach — the READINESS bar (no cold-load) holds; the VISIBLE no-pop bar does not, until S2c. The pixel-visible dot-crosses-a-boundary acceptance test MUST gate on S2b + S2c together, not S2b alone.
- **The nuance (verified):** the neighbour's box outline is drawn by the PARENT from its OWN seed-derived region roster (`child_shape`), needing no child node — so S2c is a routing problem (reach the client), not a data problem.
- **WHERE it lands / options (decide at S2c):** **(B, recommended)** the parent resolves the proxy occupant's gateway via a directory HeadRead reusing the `ParentRealmNode` resolve cadence (`update_parent_node`/`parent_headread_due` machinery) — NO wire growth; vs **(A)** grow the wire ADDITIVELY (an `OccupantInterestV2` variant carrying the gateway, with BOTH exhaustive `effect_class`/`durability_class` arms, OR a `PROTO_MINOR`-gated gateway field — NEVER a silently-appended trailing field). Then the parent emits the sibling `RealmSceneDelta` to that gateway; the agnostic client already UNIONS realms by `RealmId` (no central assembler), so sibling shapes from the parent + home shapes from the home shard compose correctly.
- **Pin:** the S2b doc comments (`RetainedOccupants`, `proxy_observer`, the `render_routes` render gate) + `scripts/vu_aoi_s2b_plan.md` §E name this companion. Flips 🟩 when a proxy-driven sibling shape reaches the client and the pixel acceptance test crosses a boundary with the neighbour already visible.

### D-RLM-12 🟥 VU-AoI — coarsen-ladder proxy compose-`Err` anti-flicker → S3 (the deep-ancestor keep-alive slice); UNREACHABLE in S2b's direct-child scope
...
- **S2+S4+S6 🟩 LANDED (2026-07-18) — PURE-RENDERER, NODE-AGNOSTIC CLIENT:** the client no longer learns which node owns any entity. **S2 (wire, minor 1→2):** appended `ServerControlMsg::OwnEntity{entity}` (the own-avatar cue by `EntityId` ALONE — no sub/node) + the reliable `EventMsg::EntityRemoved{entity}` de-owned-copy eviction arm; additive roundtrip tests, `PROTO_MINOR=2`. **S4 (gateway):** `announce_own_entity` emits `OwnEntity` on `negotiated_minor >= 2` ALONGSIDE `AuthorityChanged` (kept for old minor<2 clients) at session-attach + the dest re-point; a minor-2 client reads `OwnEntity` and IGNORES `AuthorityChanged`/`RequestCut`. **S6 (client collapse):** `DeliveredView.tracks` re-keyed `(SubId,EntityId)`→`EntityId` (ONE track per entity, latest-wins via `EntityTrack::observe`'s frame-collapse — a cross-realm re-home flips the frame node-agnostically); DELETED `authoritative_sub`/`set_authority`/`chosen_subs` + the client `cut_marker_armed` stamp + the `RequestCut`/`AuthorityChanged` handlers; ADDED `set_own_entity`/`remove_entity` + the `OwnEntity` handler. `held_subs` STAYS a SET (multi-realm AoI). Three consumers migrated in lockstep (real client `net.rs`, harness `ScriptedClient`, `bins/tests/process_parity.rs`); `devproto DevEntityRow.authoritative_sub` kept as an INERT constant (`vd_client::view::RENDERED_SUB`=0) for decode compat. `p2_transfer_gates` crossing gate re-proves the crossing as the rendered REALM-FRAME flip (System 7→8), the old two-holder sub-overlap anti-vacuity deleted (the client folds both subs into one EntityId track); `client-harness::crossing_was_real` repurposed to "a delivered track exists".
  - **🟥 OWED with S0 (send-once no-vanish, NOT in this arc):** (1) the `stub.rs::emit_frames` SEND-ONCE filter to `simulates()` only (drop the retained/fed-ghost self-emit from the CLIENT snapshot; the cross-shard COLLIDER feed `feed_source_ghosts`/`InterShardFlow::Ghost` stays) — deferred because the sim has NO server-timed no-vanish hold (S0) yet, so filtering now opens a demote→promote VANISH window (breaks `render_crossing_smoke`/`dual_cluster_crossing_smoke` no-vanish); the retained-ghost self-emit REMAINS the no-vanish mechanism. (2) The gateway `EntityRemoved` transfer-subject emit "after the dest's first Owned frame" — meaningless without send-once (there is no separate source copy to evict; emitting it now would flicker the just-re-homed avatar), so deferred with S0. (3) A dedicated reliable `MsgClass::Event` to route `EventMsg` to the client — deferred (a MsgClass add ripples into io-prod `class_to_byte`/mesh golden pins beyond this surface). The CLIENT half is REAL + tested: `ClientState::apply_event` → `DeliveredView::remove_entity` (unit-tested); the gateway emit + Event-class routing land with S0.
...
- **Still owed (1d.5b+ — the rest of the BACK half):** the real ghost-as-collider FEED + registration (`GhostFlow`
  ...
  **STILL OWED (P4/P5/P10):** (1) the NON-IDENTITY transform — swap `IdentityFrames` for the closed-form ephemeris
  ...
  un-nameable dest currently SAFE-DEGRADES to the source-frame pose
