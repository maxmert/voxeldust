...
- **REALM-UNIFICATION (D-45(a), vetted `wf_f4caf34a-ada` 2026-07-21, HOLDS_WITH_CAVEATS + adversary-survives):** everything spatial is a REALM (an authority domain: a shard owns it, it authors its DIRECT children's poses from `{input signals, last-wins}` + `{its own ambient physics}` and SHIPS them; it receives its own pose from its parent) vs an OCCUPANT (a shipped pose a realm simulates). System/Planet/Ship/Station/Area differ ONLY in parameters, never machinery (HR3). A planet is a MOVING realm = the zero-signal degenerate case; a ship is the signal-driven case. CONSEQUENCE: **the `CelestialBody` EntityKind idea is DELETED** (frozen tag space untouched) and **NO `ContinuityModel::Orbital`** (the parent ships every tick; nothing re-advances a stale realm pose; no client prediction). A realm reparent rides the EXISTING fence-CAS on its OWN `DirectoryKey::Ship/Realm` (already transfer subjects, `transfer_subject_entity`→None); `fan_out_crossing` splits on `durability_of`, never a realm-kind match. The old FA-2/FA-4b/FA-4c COLLAPSE into ONE verb — the parent authors its moving children's placements each tick and ships them — to TWO recipients on TWO lanes: OBSERVERS (render, FireAndForget latest-wins) + the CHILD SHARD (receive-own-position, an EffectFree Coupling — a SEPARATE authority lane, never smuggled onto the render datagram, HR1).
  ...
  - **owed by the arc:** FA-2b `LocalFrames::with_authored_child` (a per-tick COMPUTED placement source — today `with_moving_child` is orbital-Kepler-only + `with_placed` is static — the seam that lets a parent author a SIGNAL-driven ship, not just an orbiting planet); FA-2c parent-emits observer rows; FA-4b child-shard receive-own-position (EffectFree Coupling); FA-5 turn on moving placements + make `view.rs::world_pos` RECURSE for N-level nesting + the moving-container straddler guard (=D-45(e)); FA-6 the REGION-vs-REGION reparent DETECTOR (genuinely NEW — the occupant detector emits `DirectoryKey::Entity` subjects only) + the populated-ship atomic N+1-key directory CAS (=D-33). LEDGER-NOW owed on the containment side: (i) a voxel-built ship/station realm keeps an ANALYTIC bounding `Boundary` (Aabb/Obb) the blocks INSCRIBE for containment membership (blocks drive only render+rapier), keeping `region_signed_distance` cheap + HR3-clean; (ii) a RUNTIME register-a-realm-region path (`RealmRegions` is boot-only `::new(Vec)`, no mutator) for emergent hulls (the dynamic-registry half of (e)); (iii) D-35 `parent_version` on the snapshot to version-match a hull against its `ShipLocal` interior under datagram loss.
