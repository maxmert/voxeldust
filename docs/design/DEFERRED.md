...
- **WHY it is NOT a regression / safe to defer:** the reported TOTAL freeze (player + world both dead because the return parked on a reaped authority) is FIXED — the crossing dest-realm keep-alive holds the return-dest System live so the return COMMITS (proven by the round-trip E2E reaching `location == "System 7"` and by the deterministic 3c `rlm.rs` twin). This residual is the NARROWER "neighbours stall after the commit" facet, newly EXPOSED by the new round-trip E2E (`a_planet_to_system_return_commits_both_rehomes_and_the_player_rides`), not caused by the reap fix (which cannot suppress emission) nor by the read-only client render changes (floating-origin S2/S3 `world_pos`).
