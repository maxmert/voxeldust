...
| 10 | **The reach** — BUILT 2026-09-04 (owner: the brightness rule, "1. agree") | `ReachStated` (minor 29): a child's reach by size and by light, on change; the parent re-bands the child at the larger — size alone when the parent draws the child's light (the galaxy) — re-inserts its leaf, refolds its own reach from two sorted multisets and restates upward when the top moved; a dormant child is banded by its planted light. | sim: ten roster pins (default bands from light, the galaxy's exemption, builder order, the own fold, stale/stranger refusals, the emitter's send-on-change through the rig); core: the light reach at a Sun and an Earth, the re-band's ratio-or-gap. **OWED:** the flight; R8's galaxy-with-one-looker and bright-body count |
| 11 | **The dots are gone** — BUILT 2026-09-04 (owner: "7. approved"; R9 step 5, after the reach) | The parent states no marker for any child; the gateway reads none. A realm in reach runs and draws itself; a realm out of reach is drawn by nobody; the star field stays as the galaxy's own look. A static roster row of a child that left the range is no longer folded into later levels. | sim: the window and relay bodies are the realm's own look alone (four pins revised); gateway: the presence gate answers look-or-placement, a marker ships no delta. **OWED:** the ninth flight |
| 12 | **The far row** — BUILT 2026-09-04 (owner: "proceed with all") | A row the exact descent cannot turn (beyond the FINE rotation reach) and whose author is the sky rides in the sky's frame; the client places it from the sky anchor as it places the star cloud. | gateway: refused without a sky frame, a far row with it, another sky admits nothing; client: the anchor's frame folds, other foreign frames stay foreign. **OWED:** the flight (`window_rotated_refused` → 0 under the galaxy) |
