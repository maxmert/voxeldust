...
//!
//! Today the kernel is the octave sum over a column (the height field); the cell field (step G1)
//! replaces it with the golden chunks' whole digests when it lands.
//!
//! **Example.** A laptop whose driver miscompiles a 64-bit shift builds the golden columns wrong
//! at start: the check counts the differing words, the log names them, the flag stays false, and
//! the player gets the CPU path — never a hill the server disagrees with.
...
impl GpuRecipeCheck {
    /// Whether the GPU path is trusted: 64-bit integers present and not one word differed.
    #[must_use]
    pub const fn trusted(self) -> bool {
        self.int64 & (self.differing == 0) & (self.columns > 0)
    }
}
...
pub fn run(device: &wgpu::Device, queue: &wgpu::Queue, body: &BodyDefinition) -> GpuRecipeCheck {
    ...
    while rung <= vd_seed::ladder::RUNG_MAX {
        ...
        if n > 0 {
            ...
            columns += n as u32;
            differing += cpu.iter().zip(gpu.iter()).filter(|(a, b)| a != b).count() as u32;
        }
        rung += 1;
    }
    GpuRecipeCheck {
...
pub fn gpu_recipe_self_check(
    ...
    if check.trusted() {
        tracing::info!(
            ...
            micros = check.micros,
            "GPU RECIPE SELF-CHECK PASSED: the GPU's columns equal the CPU's, word for word"
        );
    } else if check.int64 {
        tracing::warn!(
            columns = check.columns,
            differing = check.differing,
            "GPU RECIPE SELF-CHECK FAILED: the GPU's columns differ from the CPU's — the CPU path \
             of the same source runs on this machine"
        );
    } else {
        tracing::warn!(
