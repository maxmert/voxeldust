...
use crate::bend::{Axis, Face, axis_eq, axis_neg};
...
/// The cell across a directed edge: the partner face and the cell's `(i, j)` there.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct Crossing {
    pub face: Face,
    pub i: i32,
    pub j: i32,
    /// Whether the along-edge index changed axis (`i` on one face is `j` on the other).
    pub axis_swap: bool,
}

/// ★ THE CROSSING RULE (moved here in slice 6 from the core's shell grid): the cell across `edge`
/// of `face` at along-edge position `along`, on a face of `n_l` cells per edge. The along position
/// runs the other way when the record says so, and the partner's edge index is `n_l − 1` on a plus
/// side and `0` on a minus side. The generator's halo and the grid's neighbour step share this one
/// function, so a chunk's edge cell and its neighbour across the seam are the same cell on both
/// hosts.
#[must_use]
pub const fn across(face: Face, edge: Edge, along: i32, n_l: i32) -> Crossing {
    across_with(partner_of(face, edge), edge, along, n_l)
}

/// The crossing rule for a given record: general (it honours a reversed record) so a synthetic
/// reversed record can exercise the branch the committed table never takes.
#[must_use]
pub const fn across_with(rec: SeamRecord, edge: Edge, along: i32, n_l: i32) -> Crossing {
...
mod tests {
    ...
    #[test]
    fn a_crossing_lands_on_the_partner_face_at_the_matching_cell() {
        // +X's +u side is +Y; +Y's side toward +X is its v side (v = +X), so a cell at the +u edge of
        // +X at along-position 5 lands at (5, n−1) on +Y with the axes swapped.
        let c = across(Face::PosX, Edge::UPlus, 5, 62);
        assert_eq!(c.face, Face::PosY);
        assert_eq!((c.i, c.j), (5, 61));
        assert!(c.axis_swap);
        // And back: from +Y's v-plus side at along 5 we reach +X at (61, 5).
        let back = across(Face::PosY, Edge::VPlus, 5, 62);
        assert_eq!(back.face, Face::PosX);
        assert_eq!((back.i, back.j), (61, 5));
        // +Z's −u side is −X, whose u axis is +Z: the partner side is −X's u-PLUS side (index
        // n − 1), the along position stays on j, and no axis swaps.
        let m = across(Face::PosZ, Edge::UMinus, 7, 62);
        assert_eq!(m.face, Face::NegX);
        assert_eq!((m.i, m.j), (61, 7));
        assert!(!m.axis_swap);
        // A reversed record (none exists in the table: const-asserted) runs the along position the
        // other way.
        let rev = SeamRecord {
            dst_face: Face::NegY,
            dst_edge: Edge::VMinus,
            reversed: true,
        };
        let r = across_with(rev, Edge::UPlus, 5, 62);
        assert_eq!(r.face, Face::NegY);
        assert_eq!((r.i, r.j), (56, 0));
        assert!(r.axis_swap);
    }
