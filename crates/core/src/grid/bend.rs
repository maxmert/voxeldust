...
//! re-addresses.
//!
//! **Scalars in the bend.** The bend and the seam run on `f64` and `[f64; 3]`. The boundary to the
//! position lattice (`metres_of`, `LatticePos::from_metres`) goes through `glam::DVec3`, whose
//! component-wise `f64` arithmetic is plain today; the pinned glam version therefore belongs in the
//! world identity's dependency list (the generator topic, slice 5).
