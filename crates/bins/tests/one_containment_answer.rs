...
const WHY_THESE_BITS_LAST_MOVED: &[&str] = &[
    "# \u{2605} WHY THESE BITS LAST MOVED — regenerated 2026-08-28 (slice S12, THE GALAXY'S SHAPE).",
    "#   ONE change, and it moved EVERY row. The previous record (2026-08-26, slice S9, THE CLIMB)",
    "#   is superseded.",
    "#",
    "#   THE CAUSE — THE PLACEMENT BECAME A SHAPE. A star system used to sit on a SHELL: one fixed",
    "#     radius for every sibling, and a direction drawn uniformly on a sphere. The owner refused",
    "#     that by name (owner_decisions_2026-08-27_galaxy_shape.md): a shell puts every star at the",
    "#     SAME distance, and no galaxy looks like that. The placement now draws a population, a",
    "#     radius on a density profile, an azimuth that follows a logarithmic arm, both halves of a",
    "#     triangular arm scatter, and a height above the disc.",
    "#",
    "#   WHY EVERY ROW MOVED, IN TWO STEPS.",
    "#     1. THE SYSTEM ROWS moved because the law that places them changed. The home system still",
    "#        sits at the galactic origin and its row is unmoved; every sibling is somewhere new.",
    "#     2. THE PLANET AND MOON ROWS moved because the placement now takes SIX draws from the",
    "#        seed stream where the shell took two. Every draw after them shifted by four, so each",
    "#        star drew a different mass, and every orbit, shell and SOI is solved from that mass.",
    "#",
    "#   \u{2605} AND THE CENSUS MOVED WITH IT — the visible consequence, stated so a reader does not",
    "#     have to count the lines. A lighter planet holds no moon. On seed 0 the moon count falls",
    "#     6 -> 4; on the dev cluster's home seed it falls 22 -> 17. The planet count (27) and the",
    "#     star count (3) are unchanged: nothing about HOW MANY children a parent has moved, only",
    "#     what they drew.",
    "#",
    "#   \u{2605} WHAT IS *NOT* A CAUSE, STATED BECAUSE SILENCE WOULD BE READ AS ONE. No wire type,",
    "#     no codec, no lattice tier and no frame changed in this slice. The outer radii in the",
    "#     `outer:` line above are byte-identical to the S9 record, and so is the derived mass cap.",
];
